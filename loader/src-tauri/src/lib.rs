...
async fn install_patch(
    ...
    .await
    .map_err(|error| LoaderError::Other(format!("Kurulum worker thread'i sonlandi: {error}")))?;

    if let Err(ref error) = result {
        let _ = logging::event("error", "install", &format!("Kurulum basarisiz: {error}"));
    }
    result
}
...
async fn uninstall_patch(
    ...
    let force = force.unwrap_or(false);
    let result = tauri::async_runtime::spawn_blocking(move || {
        patch::engine::uninstall(game_id, &PathBuf::from(game_root), force)
    })
    .await
    .map_err(|error| LoaderError::Other(format!("Yama kaldirma worker thread'i sonlandi: {error}")))?;

    if let Err(ref error) = result {
        let _ = logging::event("error", "uninstall", &format!("Yama kaldirma basarisiz: {error}"));
    }
    result
}
...
async fn verify_installation(
    ...
    .await
    .map_err(|error| LoaderError::Other(format!("Dosya dogrulama worker thread'i sonlandi: {error}")))?;

    if let Err(ref error) = result {
        let _ = logging::event("error", "verify", &format!("Dosya dogrulama basarisiz: {error}"));
    }
    result
}
...
fn open_external(url: String) -> Result<(), LoaderError> {
    let parsed = reqwest::Url::parse(&url)
        .map_err(|_| LoaderError::Other("Geçersiz bağlantı.".into()))?;
