...
function progress(event:OperationProgress,bad=false){const detail=event.total_bytes?`<small>${size(event.downloaded_bytes)} / ${size(event.total_bytes)} · ${event.bytes_per_second?size(event.bytes_per_second)+"/sn":""}</small>`:"";document.querySelector("#modal-root")!.innerHTML=`<div class="modal-backdrop"><section class="operation"><div class="spinner ${bad?"bad":""}"></div><h2>${escapeHtml(event.message)}</h2>${detail}<div class="operation-progress"><i style="width:${event.percent}%"></i></div><b>%${event.percent}</b>${bad?'<button class="ghost modal-close">Kapat</button>':""}</section></div>`;document.querySelector(".modal-close")?.addEventListener("click",closeModal)}
