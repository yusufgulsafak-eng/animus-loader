...
final class ApiController
{
    ...
    private function deviceInput(array $body): array
    {
        $deviceUuid=(string)($body['device_id']??'');
        $deviceName=(string)($body['device_name']??'Windows PC');
        if($deviceUuid==='')Http::error('Cihaz kimliği gerekli.',422);
        return [$deviceUuid,$deviceName];
    }

    private function login(): never
    {
        RateLimiter::enforce('login',10,60);
        $b=Http::body();
        [$deviceUuid,$deviceName]=$this->deviceInput($b);
        $user=$this->auth->login((string)($b['email']??''),(string)($b['password']??''));
        $device=(new DeviceService())->activateForUser((int)$user['id'],$deviceUuid,$deviceName);
        $token=$this->auth->issueApiToken((int)$user['id'],'loader',(int)$device['id']);
        Http::json(['ok'=>true,'data'=>['user'=>$user,'token'=>$token,'device'=>['id'=>(int)$device['id'],'device_uuid'=>$device['device_uuid'],'device_name'=>$device['device_name'],'status'=>'active']]]);
    }
    ...
    private function register(): never
    {
        ...
        $body=Http::body();
        [$deviceUuid,$deviceName]=$this->deviceInput($body);
        $this->auth->register((string)($body['email']??''),(string)($body['display_name']??''),(string)($body['password']??''));
        $user=$this->auth->login((string)($body['email']??''),(string)($body['password']??''));
        $device=(new DeviceService())->activateForUser((int)$user['id'],$deviceUuid,$deviceName);
        $token=$this->auth->issueApiToken((int)$user['id'],'loader',(int)$device['id']);
        Http::json(['ok'=>true,'data'=>['message'=>'Hesap oluşturuldu.','user'=>$user,'token'=>$token,'device'=>['id'=>(int)$device['id'],'device_uuid'=>$device['device_uuid'],'device_name'=>$device['device_name'],'status'=>'active']]],201);
    }
    ...
    private function deviceCurrent():never{$u=$this->auth->requireUser();Http::json(['ok'=>true,'data'=>(new DeviceService())->currentForUser((int)$u['id'])]);}
    private function deviceRevoke():never{$u=$this->auth->requireUser();(new DeviceService())->revokeCurrent((int)$u['id']);Http::json(['ok'=>true,'data'=>['message'=>'Cihaz bağlantısı kaldırıldı.']]);}
    ...
    private function downloadToken(int$id):never
    {
        ...
        $ttl=Env::int('DOWNLOAD_TOKEN_TTL',300);
        if(($archive['source_type']??'server')==='external'){
            if(empty($archive['external_url']))Http::error('Harici patch URL tanımlı değil.',500);
            Http::json(['ok'=>true,'data'=>['url'=>$archive['external_url'],'expires_in'=>$ttl,'source_type'=>'external','device_verified'=>true]]);
        }
        $plain=bin2hex(random_bytes(32));
        $pdo->prepare('INSERT INTO download_tokens(user_id,patch_archive_id,token_hash,expires_at) VALUES(?,?,?,DATE_ADD(NOW(),INTERVAL ? SECOND))')->execute([$u['id'],$archive['id'],hash('sha256',$plain),$ttl]);
        Http::json(['ok'=>true,'data'=>['url'=>rtrim(Env::get('APP_URL',''),'/').'/api/download/'.$plain,'expires_in'=>$ttl,'source_type'=>'server','device_verified'=>true]]);
    }
    ...
    {
        $row=Database::connection()->query('SELECT * FROM loader_config WHERE id=1')->fetch()?:['app_name'=>'Animus Profesyonel Çeviri','accent_color'=>'#B7F34A','library_title'=>'Kütüphane'];
